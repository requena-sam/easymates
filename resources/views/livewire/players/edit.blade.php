<div class="space-y-6">
    <h2 class="text-2xl font-bold text-gray-900 mb-6">{{ __('Edit player') }}</h2>
    <form wire:submit.prevent="update" class="space-y-6">
        <x-form.input-text id="pseudo" placeholder="Ex: Vanyak" required="true">
            Pseudo joueur
        </x-form.input-text>

        <x-form.input-text id="first_name" placeholder="Vanya">
            Prénom
        </x-form.input-text>

        <x-form.input-text id="last_name" placeholder="Vanya">
            Nom
        </x-form.input-text>
        ...
        <x-form.input-text id="player_number" placeholder="1">
            Numero du joueur
        </x-form.input-text>

        <x-form.input-text id="role" placeholder="Duellist, SMG,...">
            Role du joueur
        </x-form.input-text>
                ...
                <div
                    ...
                    <div class="text-center">
                        @if ($image)
                            ...
                            <p class="text-sm text-gray-500 mt-2">Cliquer pour changer</p>
                        @elseif($existing_image_uuid)
                            <div class="relative inline-block">
                                <img
                                    src="{{ app(\App\Services\ImageService::class)->getUrl($existing_image_uuid, 'medium') }}"
                                    class="h-32 mx-auto rounded-lg object-cover">
                            </div>
                            <p class="text-sm text-gray-500 mt-2">Cliquer pour changer</p>
                        @else
