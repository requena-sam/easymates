<div
    ...
    @alert-shown.window="setTimeout(() => { $wire.closeAlert() }, 5000)"
    class="fixed top-12 right-12 z-[60] max-w-md"
    style="display: none;"
>
    @php
        $colors = [
    ...
    <div class="flex items-start gap-3 p-4 {{ $currentColor['bg'] }} border-l-4 {{ $currentColor['border'] }} rounded-lg shadow-lg">
        <div class="{{ $currentColor['text'] }} flex-shrink-0">
            {!! $currentColor['icon'] !!}
        </div>
        <div class="flex-1 {{ $currentColor['text'] }}">
            <p class="text-sm font-medium">{{ $message }}</p>
        </div>
        <button
            wire:click="closeAlert"
            class="{{ $currentColor['text'] }} hover:opacity-70 transition flex-shrink-0">
